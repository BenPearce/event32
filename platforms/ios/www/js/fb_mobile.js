...
tx.executeSql("INSERT INTO EVENTS ('eventFbId','touched','start_time','update_time','end_time','description','name') VALUES (?,?,?,?,?,?,?)",[eventAttrParse[i].eid,todaysStamp,eventAttrParse[i].start_time,eventAttrParse[i].update_time,eventAttrParse[i].end_time,"testDescription","testDescription"]);
 }
                                          }, errorCB, function(){
                                                          alert("suck-ces");
                                                          var db7 = window.openDatabase("Database", "1.0", "Cordova Demo", 200000);
